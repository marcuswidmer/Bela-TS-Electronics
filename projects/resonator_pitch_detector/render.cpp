        ...
        if (analogInCntr >= updateInterval) {
            ...
            for (int i = 0; i < NUM_NOTES; i++) {
                for (int j = 0; j < NUM_OCTAVES; j++) {
                    ...
                    if (rms > rmsThreshold) {
                        midi.writeNoteOn(0, 3 * 12 + 21 + i + (j * NUM_NOTES), rms * 127);
                        sineGenerator.setNotePlaying(i,j,true);
                    } else if (sineGenerator.notePlaying(i,j)) {
                        midi.writeNoteOff(0, 3 * 12 + 21 + i + (j * NUM_NOTES), rms * 127);
                        sineGenerator.setNotePlaying(i,j,false);
                    }
                    // if (rms > biggestRms) {
                    //     biggestRms = rms;
                    //     biggestRmsNote = i;
